...
class SoraniNormalizer:
    ...
    def expand_dates_and_times(self, text: str) -> str:
        ...
        # Date e.g., 2026/08/21 or 2026-08-21
    ...
    def expand_phone_numbers(self, text: str) -> str:
        """Expand phone numbers digit-by-digit with Kurdish pronunciation."""
        def _phone(m: re.Match) -> str:
            digits = m.group(0).replace('-', '').replace(' ', '').replace('+', '')
            words = []
            for d in digits:
                if d.isdigit():
                    words.append(self.ONES[int(d)])
            return ' '.join(words)

        # Iraqi mobile: 07xx xxx xxxx
        text = re.sub(r'0\d{3}[\s-]?\d{3}[\s-]?\d{4}', _phone, text)
        # International: +964 xxx xxx xxxx
        text = re.sub(r'\+\d{1,3}[\s-]?\d{2,4}[\s-]?\d{3,4}[\s-]?\d{3,4}', _phone, text)
        return text

    def expand_urls_and_emails(self, text: str) -> str:
        """Replace URLs and email addresses with spoken descriptions."""
        # URLs
        def _url(m: re.Match) -> str:
            ...
            parts = domain.replace('.', ' دۆت ')
            return parts

        text = re.sub(r'https?://[^\s,،؛]+', _url, text)
        text = re.sub(r'www\.[^\s,،؛]+', _url, text)

        # Emails
        def _email(m: re.Match) -> str:
            local = m.group(1)
            domain = m.group(2)
            domain_spoken = domain.replace('.', ' دۆت ')
            return f"{local} ئەت {domain_spoken}"

        text = re.sub(r'([a-zA-Z0-9_.+-]+)@([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})', _email, text)
        return text

    def expand_acronyms(self, text: str) -> str:
        """Expand common English and Kurdish acronyms."""
        acronym_map = {
            "GPS": "جی پی ئێس",
            "USB": "یو ئێس بی",
            "WiFi": "وایفای",
        ...
        for acr, expanded in acronym_map.items():
            text = re.sub(r'\b' + re.escape(acr) + r'\b', expanded, text)
        return text

    def segment_sentences(self, text: str) -> List[str]:
        """Split text into sentences for batch processing and prosody control."""
        # Split on Kurdish/Arabic sentence terminators
    ...
    def normalize(self, text: str) -> str:
        ...
        text = self.expand_phone_numbers(text)
