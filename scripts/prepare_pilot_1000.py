"""
Prepare 1,000-utterance Pilot Batch from chunk_7 for VoxCPM2 LoRA Fine-Tuning.
"""
...
OUTPUT_MANIFEST = os.path.join("data", "finetune", "pilot_1000_manifest.jsonl")
WAV_DIR = os.path.join("data", "finetune", "pilot_1000_wavs")
SAMPLE_COUNT = 1000
...
def main():
    print("=" * 60)
    print(" Hawa Sorani Voice Studio — Preparing 1,000-Utterance Pilot Batch")
    print("=" * 60)
    ...
    # Read first 1000 balanced entries
    entries = []
    ...
    for e in entries:
        ...
        # Update entry with direct WAV path
        new_entry = dict(e)
    ...
    print(f"Converting {len(conversion_tasks)} MP3s to 48kHz mono WAV in parallel...")
    t0 = time.time()
    ...
    print(f"Converted {success_count}/{len(conversion_tasks)} files in {elapsed:.2f}s ({success_count/max(elapsed, 0.001):.1f} files/sec).")
    ...
    print(f"\n✓ Pilot 1,000 Manifest saved to: {OUTPUT_MANIFEST}")
